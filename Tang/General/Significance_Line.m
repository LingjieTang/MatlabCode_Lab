%% Information
%Aim: Draw draw asterisks representing significance. Horizontal and side line are optional.
%
%Prerequisite: two data, and also the position to draw horizontal line
%
%Output: Automatically draw the asterisks. A YMax that indicates the max y-value of the asterisk
%can be output for the purpose of adjusting the ylim of the figure
%
%Author: Tang Lingjie
%
%Version: 2023/03/14
...
function [YMax] = Significance_Line(x1, x2, data1, data2, p, c, FontName, FontSize, Side)
    hold on
    x = mean([x1; x2]);
    ...
    y3 = max(y1, y2) + 0.05 * (LimitY(2) - LimitY(1)); %The y position to draw horizontal line
    ...
    if (exist('Text', 'var'))
        text(x, y, Text, 'Fontname', FontName, 'FontSize', FontSize, 'Color', c, 'HorizontalAlignment', 'center', 'HandleVisibility', 'off')
    end
